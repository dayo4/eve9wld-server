...
async function sharedEmailVerificationFunction(email, request) {
    const alreadyRecorded = await knex("unverified_users")
        .where({ email })
        .select("expiration")
        .first()
    const secret = crypt.genString(8)
    const token = crypt.encrypt(JSON.stringify({ email, secret }))
    const mailOptions = mailer.templates.verifyEmail(
        `http://${
            process.env.NODE_ENV === "production" ? request.headers.host : "localhost:8080"
        }/confirmation/${token}`,
        email
    )
    ...
    if (sent) {
        if (alreadyRecorded)
            await knex("unverified_users")
                .where({ email })
                .update({ secret, expiration: Date.now() + 3600000 })
        else
            await knex("unverified_users").insert({
                email,
                secret,
    ...
    } else hlp.error("Encountered a network error, please try again. Or proceed to login")
}
...
module.exports = {
    ...
    async login(request) {
        ...
        try {
            ...
            if (foundUser) {
                ...
                if (matched) {
                    if (foundUser.account_status != "unverified") {
                        if (!foundUser.first_name) {
                            /* Means user is signing in for first time. Need to give names.*/
                            return { next: true }
                        }

                        if (foundUser.account_status === "active") {
                            const user = ({
                                id,
                                priv,
                                username,
    ...
    async verifyEmail(request) {
        try {
            ...
            if (data) {
                let user = await knex("unverified_users")
        ...
        try {
            if (stage_1) {
                ...
                if (!foundUser) {
                    ...
                    const alreadyRecorded = await knex("reset_password")
                        .where({ email })
                        .select("email")
                        .first()
                    const secret = crypt.genString(8)
                    const mailOptions = mailer.templates.resetPassword(
                        /* `http://${process.env.NODE_ENV === 'production' ? request.headers.host : 'localhost:8080'}/reset/${token}` */ secret,
                        email,
                    ...
                    if (sent) {
                        ...
                        else await knex("reset_password").insert({ email, secret, expiration })
                        return {
                            stage: 2,
                            info: "Code sent! Copy the confirmation code sent to your e-mail and paste into the input field above. Code expires in 5 minutes. Check your inbox or spam folder.",
                        }
                    } else {
                ...
                let found = await knex("reset_password")
                    .where({ email, secret: token })
                    .select("expiration")
                    .first()
                if (found)
                ...
                const verified = await knex("reset_password")
                ...
                if (verified) {
                    ...
                    const updated = await knex("users")
                        .where({ email })
                        .update({ password: hashedPassword, verified: true, active: true })
                    if (updated) {
