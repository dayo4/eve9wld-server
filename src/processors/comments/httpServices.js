...
module.exports = {
	async create(request, reply) {
		...
		try {
			const comment_id = await knex("posts_comments").insert({
				content: comment,
				user_id,
				post_id,
				parent_id
	...
	async findComments(request, reply) {
		...
		async function fetchCommentUser(comment) {
			const user = await knex
				.select("profile_image", "username", "", "last_name")
				.from("users")
	...
	async findOne(request, reply) {
		...
		try {
			...
			if (comment) {
				try {
					const user = await knex
						.select("profile_image", "", "last_name")
						.from("users")
						.where("id", comment.user_id)
						.first()
					reply.send({
	...
	async update(request, reply) {
		...
		try {
			if (content) {
				...
				const updated = await knex("posts_comments")
					.where("id", comment_id)
	...
	async delete(request, reply) {
		...
		try {
			const deleted = await knex("posts_comments")
				.where("id", comment_id)
				.andWhere("user_id", user_id)
				.del()
			if (deleted) reply.send(`Comment deleted successfully.`)
		} catch (e) {
			reply
				.status(500 /* 401 */)
				.send({error: e, message: "Unable to delete content. No comment was deleted."})
		}
